...
async function load(){ $('refresh').disabled=true; try {const response=await fetch('data/jobs.json?t='+Date.now(),{cache:'no-store'});if(!response.ok)throw Error();const result=await response.json();if(!Array.isArray(result.jobs)||!Array.isArray(result.sources))throw Error();data=result;
 ...
 $('sources').replaceChildren();for(const s of [...data.sources].sort((a,b)=>(b.jobs7d||0)-(a.jobs7d||0))){const row=el('div',undefined,'source');row.append(link(s.name,s.url),el('div',(s.ok?'✓ ':'! ')+s.message+(s.ok?` · новых: ${s.newCount||0} · вакансий за сутки: ${s.jobs24h||0} · проверено постов: ${s.postsScanned||0}`:'')));row.append(el('p',`За 7 дней: ${s.jobs7d||0} уникальных заказов · ${s.exclusive7d||0} только здесь · ${s.shared7d||0} также в других источниках`,'source-value'));if(s.jobs7d===0)row.append(el('p','Нет подходящих находок за неделю — кандидат на замену после проверки полноты сбора.','unknown'));if(s.latestJobAt)row.append(el('div','Последняя вакансия: '+new Date(s.latestJobAt).toLocaleString('ru-RU'),'unknown')); if(s.note)row.append(el('p',s.note,'unknown')); $('sources').append(row);}render();
