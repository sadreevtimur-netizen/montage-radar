...
function render() {
 ...
 const jobs=data.jobs.filter(j=>{const st=states[j.id]||'';return (!query||(j.title+' '+j.text).toLowerCase().includes(query))&&(!format||j.formats.includes(format))&&(!$('pay').checked||j.pay.length)&&((Date.now()-Date.parse(j.date))/86400000<=Number($('age').value))&&(status==='all'||status==='new'&&!st||st===status);});
 ...
 const tags=el('div',undefined,'tags'); j.formats.forEach(f=>tags.append(el('span',f,'tag')));body.append(tags);
 ...
 const actions=el('div',undefined,'actions');for(const [s,label] of [['saved','Сохранить'],['sent','Написал'],['hidden','Скрыть']]){const b=el('button',(states[j.id]===s?'✓ ':'')+label);b.setAttribute('aria-pressed',String(states[j.id]===s));b.addEventListener('click',()=>setState(j.id,s));actions.append(b);}aside.append(actions);card.append(body,aside);$('jobs').append(card);
 }
}
async function load(){ $('refresh').disabled=true; try {const response=await fetch('data/jobs.json?t='+Date.now(),{cache:'no-store'});if(!response.ok)throw Error();const result=await response.json();if(!Array.isArray(result.jobs)||!Array.isArray(result.sources))throw Error();data=result;
 ...
 $('sources').replaceChildren();for(const s of data.sources){const row=el('div',undefined,'source');row.append(link(s.name,s.url),el('div',(s.ok?'✓ ':'! ')+s.message+(s.ok?` · найдено ${s.count}`:'')));$('sources').append(row);}render();
 ...
 for(const id of ['q','format','status','age','pay']) $(id).addEventListener('input',render);
